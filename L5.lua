      ...
      if L5_env.backBuffer then
        ...
        if L5_env.filterOn then
          if L5_env.filter == "blur_twopass" then
            ...
            if
              not L5_env.blurTempCanvas
              or L5_env.blurTempCanvas:getWidth() ~= love.graphics.getWidth()
              or L5_env.blurTempCanvas:getHeight() ~= love.graphics.getHeight()
            then
...
function love.resize(w, h)
  ...
  if L5_env.backBuffer then
    L5_env.backBuffer:release()
  end
  if L5_env.frontBuffer then
    L5_env.frontBuffer:release()
  end
...
function define_env_globals()
  ...
  L5_env.lastModified = love.filesystem.getInfo("main.lua") and love.filesystem.getInfo("main.lua").modtime or 0
  L5_env.checkReloadTimer = 0
...
function lightness(_color)
  ...
  if isNormalized then
    ...
    elseif L5_env.color_mode == HSL then
      ...
      r, g, b =
        HSLtoRGB(_color[1] / L5_env.color_max[1], _color[2] / L5_env.color_max[2], _color[3] / L5_env.color_max[3])
    elseif L5_env.color_mode == HSB then
      -- Raw HSB array - convert to RGB first
      r, g, b =
        HSVtoRGB(_color[1] / L5_env.color_max[1], _color[2] / L5_env.color_max[2], _color[3] / L5_env.color_max[3])
    end
  end
...
function lerpColor(_c1, _c2, _amt)
  ...
  if c1_normalized then
    c1 = {
      _c1[1] * L5_env.color_max[1],
      _c1[2] * L5_env.color_max[2],
      _c1[3] * L5_env.color_max[3],
      _c1[4] * L5_env.color_max[4],
    }
  else
  ...
  if c2_normalized then
    c2 = {
      _c2[1] * L5_env.color_max[1],
      _c2[2] * L5_env.color_max[2],
      _c2[3] * L5_env.color_max[3],
      _c2[4] * L5_env.color_max[4],
    }
  else
...
function textAlign(x_alignment, y_alignment)
  ...
  if
    y_alignment and (y_alignment == TOP or y_alignment == CENTER or y_alignment == BOTTOM or y_alignment == BASELINE)
  then
...
function love.graphics.draw(drawable, x, y, r, sx, sy, ox, oy, kx, ky)
  ...
  if type(actualDrawable) == "userdata" and (actualDrawable:type() == "Image" or actualDrawable:type() == "Video") then
...
function blend(source, sx, sy, sw, sh, dx, dy, dw, dh, blendMode)
  ...
  elseif blendMode == REPLACE then
    ...
    error(
      "Unknown blend mode "
        .. tostring(blendMode)
        .. ". Must be of type: BLEND, NORMAL, ADD, MULTIPLY, SCREEN, LIGHTEST, DARKEST, REPLACE."
    )
  end
