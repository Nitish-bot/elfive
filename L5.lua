			...
			for key, value in pairs(tbl) do
				if type(key) ~= "number" or key > #tbl then
					...
					if type(value) == "table" then
						table.insert(
							lines,
							indent .. "  " .. keyStr .. " = " .. serializeTable(value, indent .. "  ") .. ","
						)
					else
...
function lightness(_color)
	...
	if isNormalized then
		...
		elseif L5_env.color_mode == HSL then
			...
			r, g, b = HSLtoRGB(
				_color[1] / L5_env.color_max[1],
				_color[2] / L5_env.color_max[2],
				_color[3] / L5_env.color_max[3]
			)
		elseif L5_env.color_mode == HSB then
...
function textAlign(x_alignment, y_alignment)
	...
	if
		y_alignment
		and (y_alignment == TOP or y_alignment == CENTER or y_alignment == BOTTOM or y_alignment == BASELINE)
	then
